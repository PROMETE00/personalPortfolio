...
// ⚠️ Sólo si tienes el plugin Club GreenSock:
...
const Shuffle: React.FC<ShuffleProps> = (props) => {
  ...
  // dentro de Shuffle.tsx
useEffect(() => {
  if (!ref.current) return;
  const el = ref.current;

  // reconstruye cuando cambie el tamaño del elemento
  const ro = new ResizeObserver(() => {
    // mata y vuelve a crear
    try {
      // mismo teardown que ya tienes
      tlRef.current?.kill();
      tlRef.current = null;
    ...
    // vuelve a construir + reproducir
    // (puedes factorizar a funciones si prefieres)
    // build() + play() + armHover()...
    // TIP: puedes disparar un pequeño setTimeout(0) si hace falta
  });
  ...
  useGSAP(() => {
    ...
    const play = () => {
      ...
      const addTween = (targets: HTMLElement[], at: number) => {
        tl.to(targets, {
          ...
          duration, ease, force3D: true, stagger: animationMode === 'evenodd' ? stagger : 0,
        }, at);
    ...
    return () => { st.kill(); ref.current?.removeEventListener('mouseenter', hoverHandlerRef.current as any); cleanupToStill(); setReady(false); };
  }, {
    dependencies: [text, duration, maxDelay, ease, threshold, rootMargin, fontsLoaded, shuffleDirection, shuffleTimes, animationMode, loop, loopDelay, stagger, colorFrom, colorTo, triggerOnce, respectReducedMotion, triggerOnHover],
    scope: ref,
  });
